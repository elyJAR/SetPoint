 ...
 */
export function getRowStatus(crushDateISO: string): RowStatus {
  const today = new Date().toISOString().slice(0, 10);
  if (crushDateISO < today) return 'past';
  if (crushDateISO === today) return 'today';
  return 'future';
}
 ...
 */
export function daysUntilCrush(crushDateISO: string): number {
