...
function createIDBMock() {
    ...
    const makeStore = () => ({
        ...
        put: (row) => { store[row.id] = row; return makeRequest(undefined); },
    ...
    const db = {
        ...
        transaction: (_, _mode) => makeTx(),
    };
    const openReq = { result: db, error: null, onsuccess: null, onerror: null, onupgradeneeded: null };
...
const arbRow = fc.record({
    ...
    casting_date: fc.date({ min: new Date('2000-01-01'), max: new Date('2099-12-31') }).map((d) => d.toISOString().slice(0, 10)),
    curing_duration: fc.integer({ min: 1, max: 365 }),
    curing_offset: fc.integer({ min: 0, max: 7 }),
    crush_date: fc.date({ min: new Date('2000-01-01'), max: new Date('2099-12-31') }).map((d) => d.toISOString().slice(0, 10)),
});
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
...
describe('Property: save/load round-trip', () => {
    ...
    it('produces deeply equal schedule for any valid ScheduleRow array', async () => {
        const { save, load } = await import('../src/storage');
        await fc.assert(fc.asyncProperty(fc.array(arbRow, { minLength: 0, maxLength: 10 }), async (rows) => {
            await save(rows);
