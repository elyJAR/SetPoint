...
describe('Property 11: CSV Round-Trip', () => {
    it('generateCSV then parseCSV produces equivalent rows', () => {
        ...
        fc.assert(fc.property(fc.array(arbValidRow, { minLength: 1, maxLength: 20 }), (rows) => {
            const csv = generateCSV(rows);
            const result = parseCSV(csv);
            expect(result.skipped).toHaveLength(0);
...
describe('Property 12: CSV Import Skips Invalid Rows', () => {
    it('added.length + skipped.length === total data rows', () => {
        ...
        const arbValidDataRow = arbValidRow.map(row => `${row.sample_label},${formatDisplayDate(row.casting_date)},${row.curing_duration},${row.curing_offset},${formatDisplayDate(row.crush_date)}`);
        ...
        fc.assert(fc.property(fc.array(fc.oneof(arbValidDataRow.map(r => ({ row: r, valid: true })), arbInvalidDataRow.map(r => ({ row: r, valid: false }))), { minLength: 1, maxLength: 20 }), (rowEntries) => {
